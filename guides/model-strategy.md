...
| **Cursor** | `Shift+Tab` in the agent input (editor); plans render as an editable Markdown file you can save to `.cursor/plans/` | Model picker dropdown (editor); `--model` flag or `/model` command (CLI) | Strongest available model for planning → a faster model for building |
| **GitHub Copilot** | The "Plan" agent in Copilot Chat's agent picker explores the codebase and produces a reviewable plan before any edits | Model picker dropdown in the Chat view — no slash command | Strongest available model for planning → a faster model for building |
